...
typedef struct tvec_t_base_s {
	...
	int 					pending_count;  /* active timer count, fast-forward when zero */
	tvec_root_t 			tv1;
	tvec_t 				tv2;
	tvec_t 				tv3;
	tvec_t 				tv4;
...
static inline void __run_timers(tvec_base_t *base)
{
	...
	/* Fast-forward timer_jiffies when no pending timers to avoid idle catch-up */
	if (base->pending_count == 0 && time_after_eq(jiffies, base->timer_jiffies)) {
		base->timer_jiffies = jiffies;
		return;
	}
	...
	while (time_after_eq(jiffies, base->timer_jiffies)) {
		...
		/* Timer callbacks may have removed all remaining timers, fast-forward and exit early */
		if (base->pending_count == 0 && time_after_eq(jiffies, base->timer_jiffies)) {
			base->timer_jiffies = jiffies;
			break;
		}
